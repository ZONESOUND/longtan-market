...
import {playList, textList, textInfo} from './const';
...
$('.players').click(function() {
    //player.start();
    clickPlayer(parseInt($(this).attr('id').split('-')[1]));
})

function clickPlayer(pid = 1) {
    console.log('click player!', pid);
    noSleep.enable();
...
function play(time) {
    ...
    //CHANGE: menuinner no button
    changeText("#textinfo", textInfo.start);
    //$("#textinfo").html(textInfo.start);
...
function pause() {
    ...
    $("#player-"+playerid).html(text);
    changeText("#textinfo", textInfo.wait);
    //$("#textinfo").text(textInfo.wait);
